    ...
    uint64_t message_length)
{
    ...
    for (size_t i = 0; i < messages && is_running(); i++)
    {
        ...
        *timestamp = aeron_nano_clock();
        while ((position = aeron_publication_offer(
            publication, message, message_length, NULL, NULL)) < 0)
          ;

        /*aeron_main_do_work( aeron );*/
        while ( aeron_subscription_poll( subscription, fragment_handler, fragment_asm, DEFAULT_FRAGMENT_COUNT_LIMIT ) <= 0 )
          aeron_main_idle_strategy(aeron, 1);
...
int main(int argc, char **argv)
{
    ...
    aeron_async_add_publication_t *async_ping_pub = NULL;
    ...
    aeron_context_set_use_conductor_agent_invoker(context, true);
    ...
    {
        if (aeron_async_add_publication_poll(&publication, async_ping_pub) < 0)
        {
    ...
    if (aeron_fragment_assembler_create(&fragment_assembler, pong_measuring_handler, histogram) < 0)
    {
    ...
    send_ping_and_receive_pong(
        aeron, publication, subscription, null_handler, NULL, warm_up_messages, message_length);
    ...
    send_ping_and_receive_pong(
        aeron, publication, subscription, aeron_image_fragment_assembler_handler, fragment_assembler, messages, message_length);
